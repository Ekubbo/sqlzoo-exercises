...
SELECT name, continent FROM world
	WHERE continent LIKE (SELECT continent FROM world WHERE name='Argentina')
	OR continent LIKE (SELECT continent FROM world WHERE name='Australia') 
	ORDER BY name;
...
SELECT name, continent FROM world x 
	WHERE population>=(SELECT MAX(population) FROM world y WHERE y.name<>x.name AND y.continent=x.continent)*3;
