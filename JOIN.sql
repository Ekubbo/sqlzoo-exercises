...
SELECT game.team1, game.team2, goal.player FROM goal JOIN game ON (game.id=goal.matchid) 
...
SELECT mdate, eteam.teamname FROM game JOIN eteam ON (team1=eteam.id)
...
SELECT stadium, COUNT(player) FROM goal JOIN game ON ( game.id = goal.matchid) 
	GROUP BY stadium;
...
SELECT goal.matchid, game.mdate, COUNT(goal.player) FROM game JOIN goal ON (goal.matchid=game.id)
